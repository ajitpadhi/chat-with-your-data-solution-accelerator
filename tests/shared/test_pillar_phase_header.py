"""AST invariant: every `*.py` under `src/` carries a `Pillar:` / `Phase:` docstring header.
...
* Every new module/class in `src/**` opens with a docstring header
...
This gate walks every `*.py` under `src/` and asserts the module
...
# excluded -- Hard Rule #3 scopes to `src/**`.
_SCAN_ROOTS = ("src",)
...
def test_pillar_phase_header(py_file: Path) -> None:
    ...
    if docstring is None:
        pytest.fail(
            ...
            f"module under `src/**` must open with a docstring "
...
def test_scan_actually_walked_files() -> None:
    ...
    """
    assert _ALL_FILES, "no `*.py` files discovered under src/"
    ...
    assert len(_ALL_FILES) >= 90, (
        f"only {len(_ALL_FILES)} files discovered under src/ -- "
