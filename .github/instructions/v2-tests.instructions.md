...
- `tests/conftest.py` provides shared fixtures: `app`, `settings_override`, `mock_foundry_iq`, `mock_search_handler`, `mock_chat_history`.
...
- Skipping tests without a reason. Unit tests that skip need a tracked issue link in the skip reason. Integration-lane tests **may** skip with a capability reason instead (e.g. `pytest.skip("requires cosmosdb mode; not configured")` / `"requires a populated .env")`) so the lane self-disables on machines that lack the live environment.
- In-function imports — Hard Rule #17 in [.github/copilot-instructions.md](../copilot-instructions.md) requires all imports at module top, with zero carve-outs (tests included). Enforced by `tests/shared/test_imports_at_top_only.py`.
...
- **Marker + location.** Mark every live test `@pytest.mark.integration` and place it under `tests/integration/` (mirrors the `tests/smoke/` precedent). The lane is deselected by default via `addopts = "... -m 'not smoke and not integration'"`, so the standard `pytest` run is unchanged and CI stays green without an environment.
- **Real config, loaded past the stripper.** The root `tests/conftest.py` autouse fixture strips every `AZURE_*` / `CWYD_*` env for unit isolation. The lane's own `tests/integration/conftest.py` re-loads the real `.env` (via the already-present `python-dotenv`) and calls `get_settings.cache_clear()` **after** that stripper, then session-skips the whole lane when `.env` or its required keys are absent.
...
- **Running it.** `uv run --env-file .env pytest -m integration tests/integration -v` from the repo root, with `az login` completed and `.env` populated. Mutating tests (chat-history writes) must clean up after themselves.
